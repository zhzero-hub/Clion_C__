...
int Initiate(CSNode * &T, FILE *fp)
{
    ...
    for(i = 0; i < n; i++)
    {
        ...
        fwrite(&q->data, sizeof(Node), 1, fp);
    }
    return 1;
}

void Print(CSNode * T)  //输出成员的信息
{
    cout << endl;
...
void Find_gen(CSNode * T, char na[10], int &m, int i)
{
    if(T)
    {
...
void PreOrder_(CSNode * T, CSNode * &p, char na[10])   //利用先序遍历根据姓名查找
{
    if(T)
    {
        if(strcmp(T->data.name, na) == 0)
            p = T;
        PreOrder_(T->fir, p, na);
        PreOrder_(T->sib, p, na);
    }
}

void PreOrder_Birth(CSNode * T, CSNode * &p, char bir[12])
{
    if(T)
    {
...
void Find_Parent(CSNode * T, CSNode * &p, CSNode * &f, char na[10])   //找该成员的父亲
{
    CSNode * c;
    if(T)
    {
        for(c = T->fir; c; c = c->sib)
...
void Delete_(CSNode * &T)   //删除成员及其后代
{
    CSNode *p, *f, *c;
    p = f = nullptr;
    char na[10];
